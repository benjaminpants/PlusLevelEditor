...
namespace BaldiLevelEditor
{
	...
	public partial class PlusLevelEditor : Singleton<PlusLevelEditor>
	{
		...
		static readonly FieldInfo _useRawPosition = AccessTools.Field(typeof(CursorController), "useRawPosition");
		static readonly FieldInfo _deltaThisFrame = AccessTools.Field(typeof(CursorController), "deltaThisFrame");
		...
		private static readonly FieldInfo cc_results = AccessTools.Field(typeof(CursorController), "results");
		...
		private readonly AnalogInputData movementData = new AnalogInputData()
		{
		...
		public void UpdateHighlights()
		{
			...
			if (selectedPrefab != null)
			{
				...
				selector.prefabRotations[0].transform.localPosition = selectedPrefab.transform.forward * -10f; // X
				selector.prefabRotations[1].transform.localPosition = selectedPrefab.transform.forward * 10f; // Y
				selector.prefabRotations[2].transform.localPosition = selectedPrefab.transform.up * 10f; // Z
...
#pragma warning disable CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
			...
			{
				for (int i = 0; i < selector.prefabRotations.Length; i++)
					selector.prefabRotations[i].SetActive(false);
...
#pragma warning restore CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
			...
			else
			{
				...
				for (int i = 0; i < selector.prefabRotations.Length; i++)
					selector.prefabRotations[i].SetActive(true);
				...
				if (Singleton<InputManager>.Instance.GetDigitalInput("MouseSubmit", false))
				{
					...
					for (int i = 0; i < selector.prefabRotations.Length; i++)
					{
						if (hit.transform.parent == selector.prefabRotations[i].transform)
						{
							selRotation = i;
							break;
						}
					}
					selector.rotationIndex = selRotation;
					if (selRotation != -1)
					{
						if (!hasRotationSet)
						{
							aloneRotation = selectedPrefab.transform.rotation.eulerAngles;
							lastRotationAxis = aloneRotation[selector.rotationIndex];
							hasRotationSet = true;
		...
		Vector3 aloneRotation = Vector3.zero;
		float lastRotationAxis;
		Vector2 lastCursorPosition;

		void HandlePrefabRotate()
		{
			const float gridSnap = 5f, speedMultiplier = 1.25f;

			Vector2 cursorOffset = cursor.position - lastCursorPosition; 
			lastCursorPosition = cursor.position; // Records the last position of the cursor. If it goes to the right or up, increase axis, otherwise, decrease.

			lastRotationAxis += (cursorOffset.x + cursorOffset.y) * speedMultiplier; // finally, increase the rotation axis
			lastRotationAxis %= 360f; // Avoid going too high lol

			aloneRotation[selector.rotationIndex] = Mathf.Round(lastRotationAxis / gridSnap) * gridSnap; // Subtract the last recorded axis of the object by the cursorOffset
			selectedPrefab.transform.rotation = Quaternion.Euler(aloneRotation);
